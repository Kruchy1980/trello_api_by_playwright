...
test.describe
  ...
  test('1. Should create checkItem on checklist', async ({ request }) => {
    ...
    const response = await request.post(
      `/1/checklists/${checklistId}/checkItems?name=${expectedCheckItemName}&key=${API_KEY}&token=${TOKEN}`,
      { headers },
    );
    const responseJSON = await response.json();
    // console.log(responseJSON);
  ...
  test('2. Should create and move up checkItem on checklist', async ({
    ...
    // Act: https://api.trello.com/1/checklists/{id}/checkItems?name={name}&key=APIKey&token=APIToken'
    const response = await request.post(
      `/1/checklists/${checklistId}/checkItems?name=${expectedCheckItemName}&pos=${position}&due=${dueDate}&checked=${status}&key=${API_KEY}&token=${TOKEN}`,
      { headers },
    );
    const responseJSON = await response.json();
    // console.log(responseJSON);
  ...
  test('3. Should get checkItems from checklist', async ({ request }) => {
    ...
    const response = await request.get(
      `/1/checklists/${checklistId}/checkItems?key=${API_KEY}&token=${TOKEN}`,
      { headers },
    );
    const responseJSON = await response.json();
    // console.log(responseJSON);
  ...
  test('4. Should update and move checkItem to other checklist', async ({
    ...
    const response = await request.put(
      `/1/cards/${cardId}/checkItem/${checkItemToMoveId}?name=${updatedCheckItemName}&state=${status}&idChecklist=${checklistIdTo}&key=${API_KEY}&token=${TOKEN}`,
      { headers },
    );
    const responseJSON = await response.json();
    // console.log(responseJSON);
  ...
  test('5. Should Delete checkItem from checklist', async ({ request }) => {
    ...
    const response = await request.delete(
      `/1/checklists/${checklistId}/checkItems/${checkItemToDelete}?key=${API_KEY}&token=${TOKEN}`,
      { headers },
    );
    const responseJSON = await response.json();
    // console.log(responseJSON);
  ...
  test('6.(NP) Should get Deleted checkItem from checklist', async ({
    ...
    // Act: https://api.trello.com/1/checklists/{id}/checkItems/{idCheckItem}?key=APIKey&token=APIToken
    const response = await request.get(
      `/1/checklists/${checklistId}/checkItems/${checkItemDeleted}?key=${API_KEY}&token=${TOKEN}`,
      { headers },
    );
