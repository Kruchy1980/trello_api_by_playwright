...
test.describe.serial('Cards stickers handling - dependent tests', () => {
  ...
  test('1. Should add a sticker to a card', async ({ request }) => {
    ...
    for (let i = 0; i < stickersNames.length; i++) {
      ...
      const response = await request.post(
        `/1/cards/${createdCardsIds[i]}/stickers?image=${expectedStickerName}&top=${expectedStickerFromTop}&left=${expectedStickerFromLeft}&zIndex=${expectedStickerZIndex}&key=${API_KEY}&token=${TOKEN}`,
        { headers },
      );
      const responseJSON = await response.json();
      //   console.log(responseJSON);
  ...
  test('2. Should update sticker on card', async ({ request }) => {
    ...
    const response = await request.put(
      `/1/cards/${cardId}/stickers/${stickerId}?rotate=${expectedStickerRotateValue}&top=${expectedStickerFromTop}&left=${expectedStickerFromLeft}&zIndex=${expectedStickerZIndex}&key=${API_KEY}&token=${TOKEN}`,
      { headers },
    );
    const responseJSON = await response.json();
    // console.log(responseJSON);
  ...
  test('3. Should get a sticker field', async ({ request }) => {
    ...
    const response = await request.get(
      `/1/cards/${cardId}/stickers/${stickerId}?fields=${fields}&key=${API_KEY}&token=${TOKEN}`,
      { headers },
    );
    const responseJSON = await response.json();
    // console.log(responseJSON);
  ...
  test('4. Should delete a sticker', async ({ request }) => {
    ...
    const response = await request.delete(
      `/1/cards/${cardId}/stickers/${stickerId}?key=${API_KEY}&token=${TOKEN}`,
      { headers },
    );
    const responseJSON = await response.json();
    // console.log(responseJSON);
  ...
  test('5. Should NOT get deleted sticker', async ({ request }) => {
    ...
    // Act: 'https://api.trello.com/1/cards/{id}/stickers/{idSticker}?key=APIKey&token=APIToken'
    const response = await request.get(
      `/1/cards/${cardId}/stickers/?key=${API_KEY}&token=${TOKEN}`,
      { headers },
    );
    const responseJSON = await response.json();
    // console.log(responseJSON);
