...
test.describe('Cards comments handling - simplified factories', () => {
  ...
  test.beforeEach('Create a new comment on a card', async ({ request }) => {
    ...
    // After data destrufturing
    createdCommentText = commentText;
  });
  ...
  test('1. Should get a comment on a card', async ({ request }) => {
    ...
    const responseJSON = await response.json();
    // console.log('JSON FROM RESPONSE', responseJSON);
    // console.log('Comment Id', responseJSON[0]);
    commentActionId = responseJSON[0].id;
    // console.log(commentActionId);
  ...
  test('2. Should update a comment action on a card', async ({ request }) => {
    ...
    // Destructurization for confirming comment text
